...
#   SOURCE_VERSION       - git branch to build from (default: feature/cloudformation-v1.0.12)
#   IMAGE_TAG            - image tag for docker save (default: v1.0.12)
...
#   WS_S3_PREFIX         - Workshop Studio S3 prefix (default: 0c3265a6.../static/containers)
...
SOURCE_VERSION="${SOURCE_VERSION:-feature/cloudformation-v1.0.12-prebuilt-containers}"
IMAGE_TAG="${IMAGE_TAG:-v1.0.12}"
# Default bucket name matches setup-export-project.sh naming convention
...
WS_S3_PREFIX="${WS_S3_PREFIX:-0c3265a6-1a4a-467b-ae56-e4d019184b0e/static/containers}"
...
if [ "$SKIP_BUILD" = false ]; then
  ...
  echo "  S3 dest:    s3://$S3_DEST_BUCKET/$S3_DEST_PREFIX/"
  echo ""
  ...
  # Stream the build log URL for convenience
  ...
  while true; do
    ...
    case "$STATUS" in
      SUCCEEDED)
        ...
        echo "Tarballs at: s3://$S3_DEST_BUCKET/$S3_DEST_PREFIX/"
        echo ""
        # List what was uploaded
        aws s3 ls "s3://$S3_DEST_BUCKET/$S3_DEST_PREFIX/" \
  ...
  if [ "$DOWNLOAD" = true ]; then
    ...
    aws s3 sync "s3://$S3_DEST_BUCKET/$S3_DEST_PREFIX/" "$CONTAINERS_DIR/" \
      --profile "$AWS_PROFILE_BUILD" --region "$AWS_REGION"
