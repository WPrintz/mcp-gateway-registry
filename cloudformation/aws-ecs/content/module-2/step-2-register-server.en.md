...
- **Proxy URL** — The base URL of the upstream MCP server. The gateway proxies MCP protocol messages to this URL.
- **MCP Endpoint** — Optional override for the exact MCP protocol endpoint. When set, the gateway uses this URL directly for health checks and tool discovery instead of auto-appending `/mcp/` to the Proxy URL. Needed here because Cloudflare's endpoint is at `/mcp` (no trailing slash).
...
| AI Registry Tools | 5 | registry, management, admin, gateway, mcp-tools | ✅ Healthy |
