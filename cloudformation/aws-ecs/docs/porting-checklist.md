...
29. **Inter-service URLs and Service Connect `ClientAliases` ports must match Terraform exactly** - Keep `REGISTRY_BASE_URL`, `AUTH_SERVER_URL`, `METRICS_SERVICE_URL`, etc. byte-for-byte identical to `terraform/aws-ecs/modules/mcp-gateway/ecs-services.tf`, and set each `ServiceConnectConfiguration.Services[].ClientAliases.Port` equal to the container's real port (not 80/443). Do **not** introduce port-alias indirection (e.g., mapping port 80 → container 8080) in CFN just because HTTP "defaults" look cleaner. The Envoy sidecar's alias remapping has failed intermittently on cold start, producing `httpx` exceptions with empty `str(e)` that are hard to diagnose (symptoms: `mcpgw` `list_services`/`intelligent_tool_finder` return `{"status":"failed","error":""}` after ~10s hang). v1.0.16 commit `331964be` introduced `registry:80 → 8080` alias; v1.0.20 reverted to match Terraform's `registry:8080`. Rule: when porting inter-service URLs, diff them against `ecs-services.tf` and keep them identical.
30. **Cross-SG ingress rules for mcpgw→registry (and future cross-SG ECS task calls) must be explicitly declared in CFN** - Terraform declares an explicit `aws_vpc_security_group_ingress_rule` for `mcpgw_internal` on port 8080 (and a second one for port 7860), because mcpgw tasks live in a different SG (`McpServersSG`) than the registry (`EcsTasksSG`). The `EcsTasksIngressFromSelf` rule only covers tasks that share the `EcsTasksSG` — it does **not** cover cross-SG traffic. Symptom when missing: `Connection reset by peer` from curl inside mcpgw pointed at `http://registry:8080/` after Envoy accepts the TCP connection — SC sees nothing wrong, but the SG silently drops the application-layer bytes. v1.0.20 added `EcsTasksIngressFromMcpServers8080` in `network-stack.yaml` to match terraform's `mcpgw_internal` rule. Rule on upgrade: for every entry in terraform's `security_group_ingress_rules` referencing a *different* module's `security_group_id`, confirm a matching `AWS::EC2::SecurityGroupIngress` exists in CFN. Do not rely on the self-referencing SG rule to bridge cross-SG pairs.
