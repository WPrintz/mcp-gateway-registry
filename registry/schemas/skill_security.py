...
class SkillSecurityScanFinding(BaseModel):
    ...
    file_path: Optional[str] = Field(None, description="File where finding was detected")
    line_number: Optional[int] = Field(None, description="Line number of finding")
    ...
    analyzer: str = Field(..., description="Analyzer that detected the finding: static, behavioral, llm, meta, virustotal, ai-defense")
...
class SkillSecurityScanResult(BaseModel):
    ...
    skill_md_url: Optional[str] = Field(None, description="URL to SKILL.md")
...
class SkillSecurityScanConfig(BaseModel):
    ...
    block_unsafe_skills: bool = Field(default=True, description="Disable skills that fail security scan")
    analyzers: str = Field(default="static", description="Comma-separated list of analyzers to use")
    scan_timeout_seconds: int = Field(default=120, description="Timeout for security scans in seconds")
    llm_api_key: Optional[str] = Field(None, description="API key for LLM-based analysis")
    virustotal_api_key: Optional[str] = Field(None, description="API key for VirusTotal integration")
    ai_defense_api_key: Optional[str] = Field(None, description="API key for Cisco AI Defense")
    add_security_pending_tag: bool = Field(default=True, description="Add 'security-pending' tag to unsafe skills")
...
class SkillSecurityStatus(BaseModel):
    ...
    is_disabled_for_security: bool = Field(default=False, description="Whether skill is disabled due to security issues")
